...
company_name = "041. Siriusgatan - 54+58"
...
ROW_2024_SS   = 5
ROW_2024_noFCR  = 3
ROW_2024_SS_noFCR = 7  # in case user selects this invalid combo, map to noFCR
...
ROW_2025_SS   = 13
ROW_2025_noFCR  = 11
ROW_2025_SS_noFCR = 15  # in case user selects this invalid combo, map to noFCR


ROW_MAP = {
    # year, battery, self-sufficiency focus, fcr on/off

    ("2024", "No battery", "Off", "Ancillary off"): ROW_2024_BASE,
    ("2024", "Battery",  "Off", "Ancillary on"): ROW_2024_BAT,
    ("2024", "Battery",  "On", "Ancillary on"):  ROW_2024_SS,
    ("2024", "Battery",  "Off", "Ancillary off"): ROW_2024_noFCR,
    ('2024', 'Battery', 'On', 'Ancillary off'): ROW_2024_SS_noFCR,  # in case user selects this invalid combo, map to noFCR
    # 2025
    ("2025", "No battery", "Off", "Ancillary off"): ROW_2025_BASE,
    ("2025", "Battery",  "Off", "Ancillary on"): ROW_2025_BAT,
    ("2025", "Battery",  "On", "Ancillary on"):  ROW_2025_SS,
    ("2025", "Battery",  "Off", "Ancillary off"): ROW_2025_noFCR,
    ('2025', 'Battery', 'On', 'Ancillary off'): ROW_2025_SS_noFCR,  # in case user selects this invalid combo, map to noFCR
}

# Header / Subheader templates (battery details are filled from the selected row)
HEADER_MAP = {
    ("2024", "No battery", "Off","Ancillary off"): ("Baseline 2024", "PV installed"),
    ("2024", "Battery", "Off", "Ancillary on"): (
        "Battery I ({bat_desc})",
        "Peak Shaving, Arbitrage, Ancillary Market revenues",
    ),
    ("2024", "Battery", "On", "Ancillary on"): (
        "Battery II ({bat_desc})",
        "Self-sufficiency objective and Peak Shaving, Arbitrage, Ancillary Market revenues",
    ),
    ("2024", "Battery", "Off", "Ancillary off"): (
        "Battery II ({bat_desc})",
        "Peak Shaving and Arbitrage",
    ),
    ("2024", "Battery", "On", "Ancillary off"): (
        "Battery II ({bat_desc})",
        "Self-sufficiency objective, Peak Shaving and Arbitrage",
    ),

    ("2025", "No battery", "Off","Ancillary off"): ("Baseline 2025", "PV installed"),
    ("2025", "Battery", "Off", "Ancillary on"): (
        "Battery I ({bat_desc})",
        "Peak Shaving, Arbitrage, Ancillary Market revenues",
    ),
    ("2025", "Battery", "On", "Ancillary on"): (
        "Battery II ({bat_desc})",
        "Self-sufficiency objective and Peak Shaving, Arbitrage,Ancillary Market revenues",
    ),
    ("2025", "Battery", "Off", "Ancillary off"): (
        "Battery II ({bat_desc})",
        "Peak Shaving and Arbitrage",
    ),
    ("2025", "Battery", "On", "Ancillary off"): (
        "Battery II ({bat_desc})",
        "Self-sufficiency objective, Peak Shaving and Arbitrage",
    ),
}
...
def render_header(year: str, battery: str, ss_focus: str, fcron: str, picked: dict):
    header_tpl, subheader = HEADER_MAP[(year, battery, ss_focus, fcron)]
...
with st.sidebar:
    ...
    if NoneFCR_visible:
        Ancillary_market = st.radio(
            "Ancillary Markets",
            ["Ancillary on", "Ancillary off"],
            index=0,
            help="Prioritise self-sufficiency instead of minimum cost (requires a battery).",
        )
    else:
        Ancillary_market = "Ancillary on"  # When hidden, Ancillary is always ON
...
if NoneFCR_visible == True and battery == "No battery" and Ancillary_market == "Ancillary on":
        st.info("Ancillary markets require a battery. Switched to Off.")
        Ancillary_market = "Ancillary off"

# When NoneFCR_visible is False, ensure Ancillary is off for "No battery"
if NoneFCR_visible == False and battery == "No battery":
    Ancillary_market = "Ancillary off"
...
selected_row_index = ROW_MAP[(year, battery, self_sufficiency_focus, Ancillary_market)]
...
render_header(year, battery, self_sufficiency_focus, Ancillary_market, picked)
